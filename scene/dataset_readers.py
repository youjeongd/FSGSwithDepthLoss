...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping):
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        image_path = os.path.join(images_folder, os.path.basename(extr.name))
        image_name = os.path.basename(image_path).split(".")[0]
        # rgb_path = rgb_mapping[idx]   # os.path.join(images_folder, rgb_mapping[i 
        # 추가
        if (idx) < len(rgb_mapping):
            rgb_path = rgb_mapping[idx]
        else:
            print ("overrange: idx:", idx, " len(rgb_mapping): " ,len(rgb_mapping), " rgb_mapping: ", rgb_mapping)
            continue
        # rgb_name = os.path.basename(rgb_path).split(".")[0]
        image = Image.open(rgb_path) # 원래 PATH였음

        if not os.path.exists(rgb_path):
            print("⚠️ Missing file:", rgb_path)
            continue
...
def readColmapSceneInfo(path, images, eval, n_views=0, llffhold=8):
    ...
    print("search path:", os.path.join(path, reading_dir))
    ...
    if n_views > 0:
        ...
        print("Loaded cameras:", len(train_cam_infos), "Requested:", n_views)
